    // ===== Ticket printer: Enter to print, scroll to feed it into the hole =====
    (function(){
      const ticket = document.getElementById('ticket');
      const feedEl = document.getElementById('ticketFeed');
      const hint = document.getElementById('printerHint');
      const hero = document.getElementById('hero');
      if (!ticket || !feedEl || !hero) return;

      let printed = false;   // print animation finished
      let started = false;   // Enter pressed
      ...
      ticket.addEventListener('animationend', function(){
        ticket.classList.remove('is-printing');   // hand control to inline transform
        ticket.style.transform = 'translateY(0)';
        printed = true;
        feed();   // sync with any current scroll position
      });

      // scroll pushes the printed ticket down the channel and into the hole (1:1)
      function feed(){
        if (!printed) return;
        const max = feedEl.clientHeight;               // ticket fully consumed by the hole
        const amount = Math.min(Math.max(window.scrollY, 0), max);
        ticket.style.transform = 'translateY(' + amount.toFixed(1) + 'px)';
      }
      window.addEventListener('scroll', feed, { passive:true });
